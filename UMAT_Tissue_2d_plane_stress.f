...
c...  local variables (mostly mechanics part)
      ...
      real*8 c(6), cmat(3,3), I1b, bvoigt(6), bmat(3,3)
      real*8 Fa0(3)
...
c..................................................................
c... For plane stress f13=f23=f31=f32=0 and f33 such that detf=1
      ...
      dfgrd1(3,2) = 0
      dfgrd1(3,3) = 1/detF2d
c...  fnew = [[f11, f12,0],[f21, f22, 0],[0,0,1/detF2d]]
c..................................................................
...
c...    JT Jan 14, 2021 Added dtheta missing.
        sigmaf(1) = sigmaf(1) + (1.0/1.0)*VM*PKfiber*da(1)*dtheta 
        sigmaf(2) = sigmaf(2) + (1.0/1.0)*VM*PKfiber*da(2)*dtheta 
        sigmaf(3) = sigmaf(3) + (1.0/1.0)*VM*PKfiber*da(3)*dtheta  
        sigmaf(4) = sigmaf(4) + (1.0/1.0)*VM*PKfiber*da(4)*dtheta  
        sigmaf(5) = sigmaf(5) + (1.0/1.0)*VM*PKfiber*da(5)*dtheta  
        sigmaf(6) = sigmaf(6) + (1.0/1.0)*VM*PKfiber*da(6)*dtheta 
...
c...  no need for biso
      ...
      bmat(2,3) = bvoigt(6)
      bmat(3,2) = bvoigt(6)
c...  pressure, 0 = 2*mu0*b33 +p -> p=-2*mu0*b33
c...  Note, pressure from plane stress in the fully incompressible case
      I1b = bvoigt(1)+bvoigt(2)+bvoigt(3)
      p = -2*mu0*bvoigt(3)
c... full stress tensor 
      sigma(1) = sigmaf(1) + (2*mu0*bvoigt(1) ) + p
      sigma(2) = sigmaf(2) + (2*mu0*bvoigt(2) ) + p
      sigma(3) = sigmaf(3) + (2*mu0*bvoigt(3) ) + p
      sigma(4) = sigmaf(4) + (2*mu0*bvoigt(4) )
      sigma(5) = sigmaf(5) + (2*mu0*bvoigt(5) )
      sigma(6) = sigmaf(6) + (2*mu0*bvoigt(6) )
c...      print *, 'Sigma in 3D' 
c...      print *, sigma(1), sigma(2), sigma(3), sigma(4), sigma(5), sigma(6)
c     FULL stress, even with the zero components 
...
c...    Changing to spatial fourth order tensor, 
          IIII = 0.5*(kronmat2D(q,s)*kronmat2D(r,t)+kronmat2D(q,t)*kronmat2D(r,s))
          III = kronmat2D(q,r)*kronmat2D(s,t)
c...    JT edit April 28. I am still checking if the volumetric equations are correct
          cc(II,JJ) = 4*mu0*(1/(detF2d*detF2d))*(III+IIII)
        end do
      end do
...
c...  ABT April 2021
c...  I don't know if this stays as is, for plane stress only 3 components of the stress are non-zero
c...  Unsure if we need to reorder things when packing it for abaqus' STRESS and DDSDDE
c...  JT April 28. I changed to II=1,3 since I am treating sigma(3) as sigma12 in the equations above
c...  Yes, keeping loops over 1 to 3
      do II=1,3
...
c...  ABT aprl 2021, also changing the different cciso, ccvol, etc to just cc 
          ddsdde(II,JJ) = cc(II,JJ)+ccf(II,JJ)+0.5*(kronmat2D(q,s)*sigmamat2D(r,t)
     #                    +kronmat2D(q,t)*sigmamat2D(r,s)+kronmat2D(r,s)*sigmamat2D(q,t)
     #                    +kronmat2D(r,t)*sigmamat2D(q,s))
